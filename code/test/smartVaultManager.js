...
describe('SmartVaultManager', async () => {
  ...
  describe('RS-TESTS', async () => {
    it('RS1', async () => {
      ...
      VaultManager.connect(user).mint()
      VaultManager.connect(user).mint()

      await expect(mint).to.emit(VaultManager, 'VaultDeployed').withArgs(anyValue, user.address, EUROs.address, 1)
      expect(await VaultManager.totalSupply()).to.equal(3)

      let vaults = await VaultManager.connect(user).vaults()
      expect(vaults).to.be.length(3)
      ...
      const vault = await ethers.getContractAt('SmartVaultV3', vaults[0].status.vaultAddress)
      //await vault.connect(user).mint(user.address, ethers.utils.parseEther('0.001'))
  ...
  describe('setting admin data', async () => {
    it('allows owner to admin data', async () => {
      ...
      await expect(VaultManager.connect(user).setNFTMetadataGenerator(newGenerator.address)).to.be.revertedWith(
      ...
      await expect(VaultManager.connect(user).setSmartVaultDeployer(deployerV3.address)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      )
      await expect(VaultManager.connect(user).setProtocolAddress(LiquidationPoolManager.address)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      )
      await expect(VaultManager.connect(user).setLiquidatorAddress(LiquidationPoolManager.address)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      )
