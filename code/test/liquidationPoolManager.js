...
describe('LiquidationPoolManager', async () => {
  ...
  describe('distributeFees', async () => {
    it('distributes % of accrued EUROs fees to the pool stakers', async () => {
      ...
      await expect(LiquidationPoolManager.connect(holder2).setPoolFeePercentage(newPoolFeePercentage)).to.be.revertedWith(
      ...
      ;({ _position } = await LiquidationPool.position(holder2.address))
      expectedEUROs = calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition2Value, POOL_FEE_PERCENTAGE).add(
        calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition2Value, newPoolFeePercentage)
      )
      expect(_position.EUROs).to.equal(expectedEUROs)
      ;({ _position } = await LiquidationPool.position(holder3.address))
      expectedEUROs = calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition3Value, POOL_FEE_PERCENTAGE).add(
        calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition3Value, newPoolFeePercentage)
      )
      expect(_position.EUROs).to.equal(expectedEUROs)
      ;({ _position } = await LiquidationPool.position(holder4.address))
      expectedEUROs = calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition4Value, POOL_FEE_PERCENTAGE).add(
        calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition4Value, newPoolFeePercentage)
      )
      expect(_position.EUROs).to.equal(expectedEUROs)
      ;({ _position } = await LiquidationPool.position(holder5.address))
      expectedEUROs = calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition5Value, POOL_FEE_PERCENTAGE).add(
        calculateFeeSegment(feeBalance, poolTSTTotal, tstPosition5Value, newPoolFeePercentage)
      )
      expect(_position.EUROs).to.equal(expectedEUROs)

      expect(await EUROs.balanceOf(LiquidationPoolManager.address)).to.equal(0)
      const expectedPoolEUROs = feeBalance
        .mul(POOL_FEE_PERCENTAGE)
        .div(HUNDRED_PC) // first distribution
      ...
      // 50% should go to protocol wallet
      const expectedProtocolEUROs = feeBalance
        .mul(HUNDRED_PC.sub(POOL_FEE_PERCENTAGE))
        .div(HUNDRED_PC) // first distribution
  ...
  describe('runLiquidation', async () => {
    ...
    it('returns unpurchased liquidated assets to protocol address', async () => {
      ...
      const spentOnEth1 = discounted(ethCollateral.div(4).mul(DEFAULT_ETH_USD_PRICE).div(DEFAULT_EUR_USD_PRICE))
      const expectedWBTCPurchased1 = scaleDownTo(reverseDiscounted(eurosStake1.sub(spentOnEth1)), 8)
        .mul(DEFAULT_EUR_USD_PRICE)
        .div(DEFAULT_WBTC_USD_PRICE)
      ...
      //
      // then there is 0 EUROs left to buy USDC
      const spentOnEth2 = discounted(ethCollateral.mul(3).div(4).mul(DEFAULT_ETH_USD_PRICE).div(DEFAULT_EUR_USD_PRICE))
