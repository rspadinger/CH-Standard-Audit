...
contract SmartVaultManagerV5 is ISmartVaultManager, ISmartVaultManagerV2, Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    uint256 private lastToken; //NFT counter => icreased with each mint
    ...
    modifier onlyLiquidator() {
        ...
        //console.log("Liq: ", liquidator);
        require(msg.sender == liquidator, "err-invalid-liquidator");
        _;
    }

    function vaults() external view returns (SmartVaultData[] memory) {
        //tokenId = NFT representing a user vault => for each new vault created, a tokenId NFT is minted
        uint256[] memory tokenIds = smartVaultIndex.getTokenIds(msg.sender); //get all NFTs (Vaults) for user
    ...
    // function vault1() external view returns (uint minted, uint collat) {
    //     // struct Status {
    ...
    //     // }
    //     Status memory status = ISmartVault(smartVaultIndex.getVaultAddress(1)).status();
    //     minted = status.minted;
    //     collat = status.totalCollateralValue;
    // }
